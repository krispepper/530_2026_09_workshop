# Purpose: Manages database queries and inserts for workshop hosts, locations, workshops, and time slots.

from database.mysqlConnection import fetch_data, execute_query

def get_all_hosts():
    # Fetch all host names and IDs for UI dropdowns
    return fetch_data("SELECT id, host_name FROM hosts")

def get_all_locations():
    # Fetch all available workshop locations and their IDs
...
def create_workshop(workshop_name, host_id):
    # Insert a new workshop tied to a host ID
...
def create_timeslot(workshop_id, session_datetime, is_public, location_id):
    # Schedule a new workshop session date, time, and visibility (public or private)
